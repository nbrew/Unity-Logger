...
/// <summary>
/// This behavior script is designed to manage Session and Level information for the Logger.
...
public class SessionManager : MonoBehaviour {
	...
	/// </summary>
	void OnDestroy() {
		Logger.Instance.LevelAborted("Session Being Terminated",SceneManager.GetActiveScene().name);
	...
	public void LoadLevel(int level) {
		Logger.Instance.LevelEnd(SceneManager.GetActiveScene().name);
		restarting = false;
	...
	void OnLevelWasLoaded(int level) {
		if(!restarting) {
			Logger.Instance.LevelStart(SceneManager.GetActiveScene().name);
		}

		else {
			Logger.Instance.LevelRestart(SceneManager.GetActiveScene().name);
		}
